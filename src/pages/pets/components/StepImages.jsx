...
const StepImages = ({ formState, handleChange }) => {
  ...
  const validateImage = (file, field) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      errors[field] = '❌ Only JPG, PNG, GIF, BMP, WebP formats are allowed.';
    }

    // ❌ Max file size (5MB)
    if (file.size > 5 * 1024 * 1024) {
      errors[field] = '❌ Maximum image size is 5MB.';
    }

    // ❌ Check file name length
    if (file.name.length > 50) {
      errors[field] = '❌ File name is too long.';
    }

    // ✅ If no errors, show success message
    if (Object.keys(errors).length === 0) {
      success[field] = '✅ File format and size are correct!';
    }
                    ...
                    {!extraImagesPreview[field] && (
                      <>
                        ...
                        <Typography variant="caption">
                          {isDragActive ? '' : 'Allowed formats: JPG, PNG, GIF, BMP, WebP, Max 5MB'}
                        </Typography>
                        <Typography variant="caption" sx={{ mt: 1, fontSize: '10px', color: '#999' }}>
                          ⚠️ Drag and drop may not work properly on some mobile browsers.
                        </Typography>
                        <Typography variant="caption" sx={{ fontSize: '10px', color: '#999' }}>
                          If you have issues, use the file input field below.
                        </Typography>
                      </>
                    )}
                  </div>
