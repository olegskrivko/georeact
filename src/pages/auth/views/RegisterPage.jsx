...
const RegisterPage = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccess('Account created successfully. Please check your email and confirm it to complete registration.');
        setError('');
        setTimeout(() => {
          navigate('/login');
        }, 4000);
        ...
        if (response.status === 429) {
          ...
          setError(firstError || 'An error occurred during registration.');
        } else {
          setError(data.error || data.detail || 'An error occurred during registration.');
        }
        setSuccess('');
      }
    } catch (err) {
